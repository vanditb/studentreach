alter table public.researchers
  add column if not exists verified_faculty boolean not null default false,
  add column if not exists inferred_title text,
  add column if not exists inferred_title_confidence numeric(5,4),
  add column if not exists trusted_title_source_url text;

create index if not exists researchers_verified_faculty_idx
  on public.researchers (verified_faculty, broad_field);

update public.researchers
set
  inferred_title = coalesce(inferred_title, title),
  inferred_title_confidence = coalesce(inferred_title_confidence, source_confidence),
  title = null,
  title_normalized = null,
  is_professor = false,
  is_assistant_professor = false,
  trusted_title_source_url = null,
  verified_faculty = false
